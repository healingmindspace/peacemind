...
export default function AIPlanPanel({
  ...
  onObjectiveChange, onGenerate, onAccept, onRetry, onCancel,
}: AIPlanPanelProps) {
  const { t } = useI18n();
        ...
        <div className="bg-pm-accent-light/50 rounded-xl p-3 space-y-2">
          ...
          <div className="space-y-1.5">
            {aiPlan.steps.map((step, i) => (
              <div key={i} className="flex items-start gap-2 bg-pm-surface rounded-lg px-3 py-2">
                <span className="text-xs text-brand font-medium mt-0.5">{i + 1}.</span>
                <div className="flex-1 min-w-0">
                  <p className="text-xs text-pm-text font-medium">{step.title}</p>
                  {step.description && <p className="text-[10px] text-pm-text-tertiary">{step.description}</p>}
